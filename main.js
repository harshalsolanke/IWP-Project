...
let trendingContainer = document.querySelector(".new-trending > .product-cards");
...
function createProductBannerList(list, container) {
    list.forEach(function (el) {
        ...
        if (typeof el.discount === "number" && el.discount > 0) {
            discount.innerText = el.discount + "% OFF";
            // Calculate the discounted price
            let discountedPrice = el.mrp * (1 - el.discount / 100);
            price.innerHTML = "<span style='text-decoration: line-through;'>₹" + el.mrp + "</span><br>₹" + discountedPrice.toFixed(2);
        } else {
            discount.innerText = el.discount || "No Discount";
...
function createProductList(list, container) {
    list.forEach(function (el) {
        ...
        price.innerHTML = "₹" + discountedPrice + "<span> (" + el.discount + "% OFF)</span>";
